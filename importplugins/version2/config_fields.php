...
//plugin directory
$plugindir = core_component::get_plugin_directory('dhimport', 'version2');

require_once($plugindir.'/version2.class.php');
...
//page header
rlipimport_version2_page_setup($baseurl);
echo $OUTPUT->header();

//tabs
$tabs = rlipimport_version2_get_tabs($baseurl);
print_tabs(array($tabs), $tab);

//data from db
$mappingdata = rlipimport_version2_get_mapping($tab);

//options
$plugin = new rlip_importplugin_version2(null, false);
...
if ($data = $form->get_data()) {
    if (isset($data->submitbutton)) {
        rlipimport_version2_save_mapping($tab, $options, $data);

        //notify to the user that settings were saved
        echo $OUTPUT->heading(get_string('mappingssaved', 'dhimport_version2'));
    } else if (isset($data->reset)) {
        rlipimport_version2_reset_mappings($tab);
...
if ($resetmessage) {
    ...
    echo $OUTPUT->heading(get_string('mappingsreset', 'dhimport_version2'));
}

$cancelmessage = optional_param('cancelmessage', 0, PARAM_INT);
if ($cancelmessage) {
    //notify the user that the settings were reset to DB values
    echo $OUTPUT->heading(get_string('mappingscancelled', 'dhimport_version2'));
}
